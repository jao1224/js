...
function App() {
  ...
  const handleSubItemInputChange = (todoId, value) => {
    setSubItemInputs(prev => ({ ...prev, [todoId]: value }));
  };
    ...
    if (subItem) {
      ...
      if (!parentExists) {
        ...
        if (parentInHistory) {
          ...
          // Se a tarefa pai não existe nem no histórico, cria uma nova
          const newTodo = {
            id: subItem.parentId,
        ...
        // Se a tarefa pai existe, apenas adiciona o sub-item
        setTodos(prev =>
          prev.map(todo =>
      ...
      // Remove o sub-item do histórico
      setDeletedSubItems(prev => prev.filter(s => s.id !== subItemId));
      
      // Rola para o final da lista após restaurar
      setTimeout(scrollToBottom, 100);
    }
  };
